...
def _mock_ssl():
    ssl = patch("mailer.smtplib.SMTP_SSL")
    return ssl
...
def test_send_email_multiple_recipients(tmp_path):
    jpeg = tmp_path / "photo.jpg"
    from PIL import Image
    Image.new("RGB", (600, 400)).save(str(jpeg), "JPEG")
    ...
    with patch("mailer.smtplib.SMTP_SSL") as mock_ssl:
        ...
        send_email(
            filepath=str(jpeg),
            recipient="jan@example.cz, marie@example.cz ,petr@example.cz",
            smtp_server="smtp.seznam.cz",
            smtp_port=465,
            smtp_user="bot@seznam.cz",
            smtp_pass="secret",
        ...
        assert kwargs.get("to_addrs") == ["jan@example.cz", "marie@example.cz", "petr@example.cz"]
